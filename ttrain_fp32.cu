...
void gpt_forward(GPT* model, int* inputs, int* targets, int B, int T) {
    ...
    for (int l = 0; l < numLayers; l++) {
        ...
        attention_forward(l_atty, l_queryKeyValueResult, l_att, scratch, B, T, C, numHeads);
        ...
        residual_forward(l_residual2, residual, l_attentionProjection, B*T*C);
        ...
        gelu_forward(l_fullyConnectedGelu, l_fch, B*T*4*C);
        ...
        residual_forward(l_residual3, l_residual2, l_fullyConnectedProjection, B*T*C);
    ...
    if (targets != NULL) {
        fused_classifier3(acts.output, acts.losses, NULL, model->targets, B, T, vocabSize, paddedVocabSize);
